...
def compact(document: dict) -> dict:
    ...
    open_links = sum(
        r.get("edge_status") == "open" for r in runs if r.get("depth", 0) > 0
    )
    assessed = [r for r in runs if r.get("outcome") != "not_assessed"]
    alerts = []
    if total_tokens and coordination / total_tokens >= 0.20:
        alerts.append(
            f"Coordination is approximately {coordination / total_tokens:.0%} of observed tokens."
        )
    high_pressure = [
        r
        for r in runs
        if isinstance(r.get("latest_context_pressure"), (int, float))
        and r["latest_context_pressure"] >= 0.75
    ]
    if high_pressure:
        alerts.append(
            f"{len(high_pressure)} agent(s) are at or above 75% context pressure."
        )
    if open_links >= 4:
        alerts.append(
    ...
    return {
        ...
        "root_ref": document.get("root_ref"),
        "health": "warning" if alerts else "ok",
        "open_child_links": open_links,
        "observed_agents": len(runs),
        "assessed_agents": len(assessed),
        "gross_tokens": total_tokens,
        "estimated_credits": credits,
        "coordination_tokens_approx": coordination,
...
def html_dashboard(doc: dict, snapshot: dict, refresh: int) -> str:
    ...
    for run in doc["runs"]:
        route = run["route"]
        routes.setdefault(route, {"tokens": 0, "credits": 0, "runs": 0})
        routes[route]["tokens"] += run["gross_total_tokens"]
        routes[route]["credits"] += float(run.get("estimated_credits") or 0)
        routes[route]["runs"] += 1
    max_tokens = max([v["tokens"] for v in routes.values()] or [1])
    bars = "".join(
        f'<tr><td>{escape(k)}</td><td>{v["runs"]}</td><td>{v["tokens"]:,}</td><td>{v["credits"]:.3f}</td><td><div class="bar" style="width:{max(2, v["tokens"] / max_tokens * 100):.1f}%"></div></td></tr>'
...
def collect(args) -> tuple[dict, dict, set[Path]]:
    ...
    if not root or not CAL.AUDIT.parse_task_id(root):
        raise CAL.AUDIT.AuditError(
            "Provide a task UUID or use --current with CODEX_THREAD_ID"
        )
...
def write_cycle(args, generation: int) -> None:
    doc, snap, protected = collect(args)
    ...
    if any(path in protected for path in outputs):
        raise CAL.AUDIT.AuditError(
            "Monitor output collides with a telemetry or input path"
        )
    CAL.AUDIT.safe_write(outputs[0], json.dumps(doc, indent=2) + "\n", protected, True)
    CAL.AUDIT.safe_write(outputs[1], json.dumps(snap, indent=2) + "\n", protected, True)
    CAL.AUDIT.safe_write(outputs[2], CAL.markdown(doc), protected, True)
    CAL.AUDIT.safe_write(
        outputs[3], html_dashboard(doc, snap, max(5, args.interval)), protected, True
    )
    CAL.AUDIT.safe_write(
        outputs[4],
        json.dumps(
            {
                "status": "running" if not args.once else "complete",
                "generation": generation,
                "last_success": snap["generated_at"],
                "last_error": None,
            },
            indent=2,
        )
...
def main() -> None:
    ...
    if args.current and args.task:
        p.error("Use --current or a task UUID, not both")
    if args.interval < 5:
    ...
    while not STOP:
        generation += 1
        try:
            write_cycle(args, generation)
        except Exception as exc:
            atomic_write(
                args.output_dir.resolve() / "monitor-status.json",
                json.dumps(
                    {
                        "status": "error",
                        "generation": generation,
    ...
    if STOP:
        atomic_write(
            args.output_dir.resolve() / "monitor-status.json",
            json.dumps(
                {"status": "stopped", "generation": generation, "last_error": None},
                indent=2,
            )
            + "\n",
        )
